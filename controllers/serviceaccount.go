...
func (r *MicroserviceReconciler) checkServiceAccount(mic *microservicev1beta1.Microservice, reqLogger logr.Logger) error {
	if mic.Spec.DisableServiceAccountCreation {
		sa := &corev1.ServiceAccount{}

		err := r.Client.Get(context.TODO(), types.NamespacedName{Name: mic.GetName(), Namespace: mic.GetNamespace()}, sa)
...
func (r *MicroserviceReconciler) checkServiceAccountSecret(mic *microservicev1beta1.Microservice, reqLogger logr.Logger) error {
	if mic.Spec.DisableServiceAccountCreation {
		secretName := fmt.Sprintf("%s-sa", mic.GetName())
		secret := &corev1.Secret{}

		err := r.Client.Get(context.TODO(), types.NamespacedName{Name: secretName, Namespace: mic.GetNamespace()}, secret)
		if err != nil && k8sErrors.IsNotFound(err) {
			return nil
		} else if err != nil {
			return err
